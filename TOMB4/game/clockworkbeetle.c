...
void ControlClockworkBeetle(short item_number) {
	...
	SoundEffect(SFX_BEETLE_CLK_WHIRR, (PHD_VECTOR*)&item->pos, item->room_number, SFX_DEFAULT);
	item->fallspeed += 12;
	...
	if(item->item_flags[0]) {
		...
		if(lara.beetle_uses) {
			...
			if(bounce && rotY) {
				...
				SoundEffect(SFX_BEETLE_CLK_EXP, (PHD_VECTOR*)&item->pos, item->room_number, 0);
