...
void FullBlockSwitchCollision(short item_number, ITEM_INFO* l, COLL_INFO* coll) {
	...
	if(input & IN_ACTION && item->status == ITEM_INACTIVE && !(item->flags & IFL_INVISIBLE) && CurrentSequence < 3 && lara.gun_status == LG_NO_ARMS && l->current_anim_state == AS_STOP && l->anim_number == ANIM_BREATH || lara.IsMoving && lara.GeneralPtr == (void*)item_number) {
...
void SwitchCollision2(short item_number, ITEM_INFO* l, COLL_INFO* coll) {
	...
	if(input & IN_ACTION && item->status == ITEM_INACTIVE && lara.water_status == LW_UNDERWATER && lara.gun_status == LG_NO_ARMS && l->current_anim_state == AS_TREAD) {
...
void UnderwaterSwitchCollision(short item_number, ITEM_INFO* l, COLL_INFO* coll) {
	...
	if(input & IN_ACTION && lara.water_status == LW_UNDERWATER && l->current_anim_state == AS_TREAD && l->anim_number == ANIM_TREAD && lara.gun_status == LG_NO_ARMS && item->current_anim_state == 0 || lara.IsMoving && lara.GeneralPtr == (void*)item_number) {
		flag = 0;
...
void PulleyCollision(short item_number, ITEM_INFO* l, COLL_INFO* coll) {
	...
	if(!(item->flags & IFL_INVISIBLE) && (input & IN_ACTION && lara.gun_status == LG_NO_ARMS && l->current_anim_state == AS_STOP && l->anim_number == ANIM_BREATH && !l->gravity_status || lara.IsMoving && lara.GeneralPtr == (void*)item_number)) {
		roty = item->pos.y_rot;
...
void TurnSwitchControl(short item_number) {
	...
	if(item->item_flags[0] == 2) {
		...
		if(l->frame_number >= anims[ANIM_TURNSWITCHAB].frame_base && l->frame_number <= anims[ANIM_TURNSWITCHAB].frame_base + 43 || l->frame_number >= anims[ANIM_TURNSWITCHAB].frame_base + 58 && l->frame_number <= anims[ANIM_TURNSWITCHAB].frame_base + 115)
			SoundEffect(SFX_PUSHABLE_SOUND, &item->pos, SFX_ALWAYS);
	}
...
void RailSwitchCollision(short item_number, ITEM_INFO* l, COLL_INFO* coll) {
	...
	if(input & IN_ACTION && l->current_anim_state == AS_STOP && l->anim_number == ANIM_BREATH && lara.gun_status == LG_NO_ARMS || lara.IsMoving && lara.GeneralPtr == (void*)item_number) {
		if(item->current_anim_state == 1) {
			l->pos.y_rot ^= 0x8000;
...
void JumpSwitchCollision(short item_number, ITEM_INFO* l, COLL_INFO* coll) {
	...
	if(input & IN_ACTION && lara.gun_status == LG_NO_ARMS && (l->current_anim_state == AS_REACH || l->current_anim_state == AS_UPJUMP) && l->gravity_status && l->fallspeed > 0 && !item->current_anim_state) {
...
void CrowbarSwitchCollision(short item_number, ITEM_INFO* l, COLL_INFO* coll) {
	...
	if(input & IN_ACTION || GLOBAL_inventoryitemchosen == CROWBAR_ITEM && l->current_anim_state == AS_STOP && l->anim_number == ANIM_BREATH && lara.gun_status == LG_NO_ARMS && !item->item_flags[0] || lara.IsMoving && lara.GeneralPtr == (void*)item_number) {
		if(item->current_anim_state == 1) {
			l->pos.y_rot ^= 0x8000;
...
void CogSwitchCollision(short item_number, ITEM_INFO* l, COLL_INFO* coll) {
	...
	if(!(item->flags & IFL_INVISIBLE) && (input & IN_ACTION && lara.gun_status == LG_NO_ARMS && !l->gravity_status && l->current_anim_state == AS_STOP && l->anim_number == ANIM_BREATH || lara.IsMoving && lara.GeneralPtr == (void*)item_number)) {
