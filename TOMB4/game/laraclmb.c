
#include "game/laraclmb.h"
#include "game/animstruct.h"
#include "game/camera.h"
#include "game/collinfo.h"
#include "game/control.h"
#include "game/floortypes.h"
#include "game/heighttypes.h"
#include "game/inputbuttons.h"
#include "game/iteminfo.h"
#include "game/lara.h"
#include "game/lara_states.h"
#include "game/laragunstatus.h"
#include "game/larainfo.h"
#include "game/laramisc.h"
#include "game/levelinfo.h"
#include "game/quadrantnames.h"
#include "global/types.h"
#include "specific/input.h"
#include <stdlib.h>


static short LeftIntRightExtTab[4] = { 2048, 256, 512, 1024 };
static short LeftExtRightIntTab[4] = { 512, 1024, 2048, 256 };

void lara_as_climbstnc(ITEM_INFO* item, COLL_INFO* coll) {
	lara.IsClimbing = 1;
	coll->enable_spaz = 0;
	coll->enable_baddie_push = 0;
	camera.target_elevation = -3640;

	if(S_IsActionDown(inputImpl, IN_LOOK)) {
		LookUpDown();
	}

	if(S_IsActionDown(inputImpl, IN_LEFT) || S_IsActionDown(inputImpl, IN_LSTEP)) {
		item->goal_anim_state = AS_CLIMBLEFT;
		lara.move_angle = item->pos.y_rot - 16384;
	} else if(S_IsActionDown(inputImpl, IN_RIGHT) || S_IsActionDown(inputImpl, IN_RSTEP)) {
		item->goal_anim_state = AS_CLIMBRIGHT;
		lara.move_angle = item->pos.y_rot + 16384;
	} else if(S_IsActionDown(inputImpl, IN_JUMP)) {
		if(item->anim_number == ANIM_CLIMBSTNC) {
			item->goal_anim_state = AS_BACKJUMP;
			lara.gun_status = LG_NO_ARMS;
			lara.move_angle = item->pos.y_rot + 32768;
		}
	}
}

void lara_as_climbleft(ITEM_INFO* item, COLL_INFO* coll) {
	coll->enable_baddie_push = 0;
	coll->enable_spaz = 0;
	camera.target_angle = -5460;
	camera.target_elevation = -2730;

	if(!(S_IsActionDown(inputImpl, IN_LEFT) || S_IsActionDown(inputImpl, IN_LSTEP))) {
		item->goal_anim_state = AS_CLIMBSTNC;
	}
}

void lara_col_climbleft(ITEM_INFO* item, COLL_INFO* coll) {
	int shift, res;

	if(!LaraCheckForLetGo(item, coll)) {
		lara.move_angle = item->pos.y_rot - 16384;
		res = LaraTestClimbPos(item, coll->radius, -(coll->radius + 120), -512, 512, &shift);
		LaraDoClimbLeftRight(item, coll, res, shift);
	}
}

void lara_as_climbright(ITEM_INFO* item, COLL_INFO* coll) {
	coll->enable_baddie_push = 0;
	coll->enable_spaz = 0;
	camera.target_angle = 5460;
	camera.target_elevation = -2730;

	if(!(S_IsActionDown(inputImpl, IN_RIGHT) || S_IsActionDown(inputImpl, IN_RSTEP))) {
		item->goal_anim_state = AS_CLIMBSTNC;
	}
}

void lara_col_climbright(ITEM_INFO* item, COLL_INFO* coll) {
	int shift, res;

	if(!LaraCheckForLetGo(item, coll)) {
		lara.move_angle = item->pos.y_rot + 16384;
		res = LaraTestClimbPos(item, coll->radius, coll->radius + 120, -512, 512, &shift);
		LaraDoClimbLeftRight(item, coll, res, shift);
	}
}

void lara_as_climbing(ITEM_INFO* item, COLL_INFO* coll) {
	coll->enable_baddie_push = 0;
	coll->enable_spaz = 0;
	camera.target_elevation = 5460;
}

void lara_as_climbdown(ITEM_INFO* item, COLL_INFO* coll) {
	coll->enable_baddie_push = 0;
	coll->enable_spaz = 0;
	camera.target_elevation = -8190;
}

void lara_as_climbend(ITEM_INFO* item, COLL_INFO* coll) {
	coll->enable_baddie_push = 0;
	coll->enable_spaz = 0;
	camera.flags = 1;
	camera.target_angle = -8190;
}

void lara_col_climbstnc(ITEM_INFO* item, COLL_INFO* coll) {
	int result_r, result_l, shift_r, shift_l, ledge_r, ledge_l;

	if(LaraCheckForLetGo(item, coll) || item->anim_number != ANIM_CLIMBSTNC) {
		return;
	}

	if(S_IsActionDown(inputImpl, IN_FORWARD)) {
		if(item->goal_anim_state == AS_NULL) {
			return;
		}

		item->goal_anim_state = AS_CLIMBSTNC;
		result_r = LaraTestClimbUpPos(item, coll->radius, coll->radius + 120, &shift_r, &ledge_r);
		result_l = LaraTestClimbUpPos(item, coll->radius, coll->radius - 120, &shift_l, &ledge_l);

		if(!result_r || !result_l) {
			return;
		}

		if(result_r < 0 || result_l < 0) {
			if(abs(ledge_l - ledge_r) <= 120) {
				item->goal_anim_state = AS_NULL;
				item->pos.pos.y += (ledge_l + ledge_r) / 2 - 256;
				return;
			}
		}

		if(shift_r) {
			if(shift_l) {
				if(shift_r < 0 != shift_l < 0) {
					return;
				}

				if(shift_r < 0 && shift_r < shift_l) {
					shift_l = shift_r;
				} else if(shift_r > 0 && shift_r > shift_l) {
					shift_l = shift_r;
				}
			} else {
				shift_l = shift_r;
			}
		}

		item->goal_anim_state = AS_CLIMBING;
		item->pos.pos.y += shift_l;
	} else if(S_IsActionDown(inputImpl, IN_BACK)) {
		if(item->goal_anim_state == AS_HANG) {
			return;
		}

		item->goal_anim_state = AS_CLIMBSTNC;
		item->pos.pos.y += 256;
		result_r = LaraTestClimbPos(item, coll->radius, coll->radius + 120, -512, 512, &shift_r);
		result_l = LaraTestClimbPos(item, coll->radius, coll->radius - 120, -512, 512, &shift_l);
		item->pos.pos.y -= 256;

		if(!result_r || !result_l) {
			return;
		}

		if(shift_r && shift_l) {
			if(shift_r < 0 != shift_l < 0) {
				return;
			}

			if((shift_r > 0 && shift_r > shift_l) || (shift_r < 0 && shift_r < shift_l)) {
				shift_l = shift_r;
			}
		}

		if(result_l == 1 && result_r == 1) {
			item->goal_anim_state = AS_CLIMBDOWN;
			item->pos.pos.y += shift_l;
		} else {
			item->goal_anim_state = AS_HANG;
		}
	}
}

void lara_col_climbing(ITEM_INFO* item, COLL_INFO* coll) {
	int frame, yshift, result_r, result_l, shift_r, shift_l, ledge_r, ledge_l;

	if(LaraCheckForLetGo(item, coll)) {
		return;
	}

	if(item->anim_number != ANIM_CLIMBING) {
		return;
	}

	frame = item->frame_number - GetAnim(currentLevel, ANIM_CLIMBING)->frame_base;

	if(!frame) {
		yshift = 0;
	} else if(frame == 28 || frame == 29) {
		yshift = -256;
	} else if(frame == 57) {
		yshift = -512;
	} else {
		return;
	}

	item->pos.pos.y += yshift - 256;
	result_r = LaraTestClimbUpPos(item, coll->radius, coll->radius + 120, &shift_r, &ledge_r);
	result_l = LaraTestClimbUpPos(item, coll->radius, -(coll->radius + 120), &shift_l, &ledge_l);
	item->pos.pos.y += 256;

	if(result_r && result_l && S_IsActionDown(inputImpl, IN_FORWARD)) {
		if(result_r < 0 || result_l < 0) {
			item->goal_anim_state = AS_CLIMBSTNC;
			AnimateLara(item);

			if(abs(ledge_r - ledge_l) <= 120) {
				item->goal_anim_state = AS_NULL;
				item->pos.pos.y += (ledge_r + ledge_l) / 2 - 256;
			}
		} else {
			item->goal_anim_state = AS_CLIMBING;
			item->pos.pos.y -= yshift;
		}
	} else {
		item->goal_anim_state = AS_CLIMBSTNC;

		if(yshift) {
			AnimateLara(item);
		}
	}
}

void lara_col_climbdown(ITEM_INFO* item, COLL_INFO* coll) {
	int frame, yshift, result_r, result_l, shift_r, shift_l;

	if(LaraCheckForLetGo(item, coll)) {
		return;
	}

	if(item->anim_number != ANIM_CLIMBDOWN) {
		return;
	}

	frame = item->frame_number - GetAnim(currentLevel, ANIM_CLIMBDOWN)->frame_base;

	if(!frame) {
		yshift = 0;
	} else if(frame == 28 || frame == 29) {
		yshift = 256;
	} else if(frame == 57) {
		yshift = 512;
	} else {
		return;
	}

	item->pos.pos.y += yshift + 256;
	result_r = LaraTestClimbPos(item, coll->radius, coll->radius + 120, -512, 512, &shift_r);
	result_l = LaraTestClimbPos(item, coll->radius, -(coll->radius + 120), -512, 512, &shift_l);
	item->pos.pos.y -= 256;

	if(result_r && result_l && S_IsActionDown(inputImpl, IN_BACK)) {
		if(shift_r && shift_l && shift_r < 0 != shift_l < 0) {
			item->goal_anim_state = AS_CLIMBSTNC;
			AnimateLara(item);
		} else if(result_r == -1 || result_l == -1) {
			item->anim_number = ANIM_CLIMBSTNC;
			item->frame_number = GetAnim(currentLevel, ANIM_CLIMBSTNC)->frame_base;
			item->current_anim_state = AS_CLIMBSTNC;
			item->goal_anim_state = AS_HANG;
			AnimateLara(item);
		} else {
			item->goal_anim_state = AS_CLIMBDOWN;
			item->pos.pos.y -= yshift;
		}
	} else {
		item->goal_anim_state = AS_CLIMBSTNC;

		if(yshift) {
			AnimateLara(item);
		}
	}
}

short GetClimbTrigger(int x, int y, int z, short room_number) {
	FLOOR_INFO* floor;
	height_types ht;
	int tiltxoff, tiltzoff, OnObject;
	short* data;

	floor = GetFloor(x, y, z, &room_number);
	GetHeight(floor, x, y, z, &ht, &tiltxoff, &tiltzoff, &OnObject);

	if(!trigger_index) {
		return 0;
	}

	data = trigger_index;

	if((*data & 0x1F) == LAVA_TYPE) {
		if(*data & 0x8000) {
			return 0;
		}

		data++;
	}

	if((*data & 0x1F) == CLIMB_TYPE) {
		return *data;
	}

	return 0;
}

int LaraTestClimb(int x, int y, int z, int xfront, int zfront, int item_height, short item_room, int* shift) {
	FLOOR_INFO* floor;
	height_types ht;
	int tiltxoff, tiltzoff, OnObject;
	int hang, h, c;
	short room_number;

	*shift = 0;
	hang = 1;

	if(!lara.climb_status) {
		return 0;
	}

	room_number = item_room;
	floor = GetFloor(x, y - 128, z, &room_number);
	h = GetHeight(floor, x, y, z, &ht, &tiltxoff, &tiltzoff, &OnObject);

	if(h == NO_HEIGHT) {
		return 0;
	}

	h -= 128 + y + item_height;

	if(h < -70) {
		return 0;
	}

	if(h < 0) {
		*shift = h;
	}

	c = GetCeiling(floor, x, y, z) - y;

	if(c > 70) {
		return 0;
	}

	if(c > 0) {
		if(*shift) {
			return 0;
		}

		*shift = c;
	}

	if(item_height + h < 900) {
		hang = 0;
	}

	floor = GetFloor(xfront + x, y, zfront + z, &room_number);
	h = GetHeight(floor, xfront + x, y, zfront + z, &ht, &tiltxoff, &tiltzoff, &OnObject);

	if(h != NO_HEIGHT) {
		h -= y;
	}

	if(h > 70) {
		c = GetCeiling(floor, xfront + x, y, zfront + z) - y;

		if(c >= 512) {
			return 1;
		}

		if(c > 442) {
			if(*shift > 0) {
				if(hang) {
					return -1;
				} else {
					return 0;
				}
			}

			*shift = c - 512;
			return 1;
		}

		if(c > 0) {
			if(hang) {
				return -1;
			} else {
				return 0;
			}
		}

		if(c <= -70 || !hang || *shift > 0) {
			return 0;
		}

		if(*shift > c) {
			*shift = c;
		}

		return -1;
	}

	if(h > 0) {
		if(*shift < 0) {
			return 0;
		}

		if(h > *shift) {
			*shift = h;
		}
	}

	room_number = item_room;
	GetFloor(x, y + item_height, z, &room_number);
	floor = GetFloor(xfront + x, y + item_height, zfront + z, &room_number);
	c = GetCeiling(floor, xfront + x, y + item_height, zfront + z);

	if(c == NO_HEIGHT) {
		return 1;
	}

	c -= y;

	if(c <= h || c >= 512) {
		return 1;
	}

	if(c <= 442) {
		if(hang) {
			return -1;
		} else {
			return 0;
		}
	}

	if(*shift > 0) {
		if(hang) {
			return -1;
		} else {
			return 0;
		}
	}

	*shift = c - 512;
	return 1;
}

int LaraTestClimbPos(ITEM_INFO* item, int front, int right, int origin, int height, int* shift) {
	int angle, x, z, xfront, zfront;

	xfront = 0;
	zfront = 0;
	angle = (unsigned short)(item->pos.y_rot + 0x2000) / 0x4000;

	switch(angle) {
	case NORTH:
		x = right + item->pos.pos.x;
		z = front + item->pos.pos.z;
		zfront = 256;
		break;

	case EAST:
		x = front + item->pos.pos.x;
		z = item->pos.pos.z - right;
		xfront = 256;
		break;

	case SOUTH:
		x = item->pos.pos.x - right;
		z = item->pos.pos.z - front;
		zfront = -256;
		break;

	default:
		x = item->pos.pos.x - front;
		z = right + item->pos.pos.z;
		xfront = -256;
		break;
	}

	return LaraTestClimb(x, origin + item->pos.pos.y, z, xfront, zfront, height, item->room_number, shift);
}

int LaraTestClimbUpPos(ITEM_INFO* item, int front, int right, int* shift, int* ledge) {
	FLOOR_INFO* floor;
	height_types ht;
	int tiltxoff, tiltzoff, OnObject;
	int angle, x, y, z, xfront, zfront, h, c;
	short room_number;

	xfront = 0;
	zfront = 0;
	y = item->pos.pos.y - 768;
	angle = (unsigned short)(item->pos.y_rot + 0x2000) / 0x4000;

	switch(angle) {
	case NORTH:
		x = right + item->pos.pos.x;
		z = front + item->pos.pos.z;
		zfront = 4;
		break;

	case EAST:
		x = front + item->pos.pos.x;
		z = item->pos.pos.z - right;
		xfront = 4;
		break;

	case SOUTH:
		x = item->pos.pos.x - right;
		z = item->pos.pos.z - front;
		zfront = -4;
		break;

	default:
		x = item->pos.pos.x - front;
		z = right + item->pos.pos.z;
		xfront = -4;
		break;
	}

	*shift = 0;
	room_number = item->room_number;
	floor = GetFloor(x, y, z, &room_number);
	c = 256 - y + GetCeiling(floor, x, y, z);

	if(c > 70) {
		return 0;
	}

	if(c > 0) {
		*shift = c;
	}

	floor = GetFloor(x + xfront, y, z + zfront, &room_number);
	h = GetHeight(floor, x + xfront, y, z + zfront, &ht, &tiltxoff, &tiltzoff, &OnObject);

	if(h == NO_HEIGHT) {
		*ledge = NO_HEIGHT;
		return 1;
	}

	h -= y;
	*ledge = h;

	if(h > 128) {
		c = GetCeiling(floor, x + xfront, y, z + zfront) - y;

		if(c >= 512) {
			return 1;
		}

		if(h - c > 762) {
			*shift = h;
			return -1;
		}
	} else {
		if(h > 0 && h > *shift) {
			*shift = h;
		}

		room_number = item->room_number;
		GetFloor(x, y + 512, z, &room_number);
		floor = GetFloor(x + xfront, y + 512, z + zfront, &room_number);
		c = GetCeiling(floor, x + xfront, y + 512, z + zfront) - y;

		if(c <= h || c >= 512) {
			return 1;
		}
	}

	return 0;
}

int LaraCheckForLetGo(ITEM_INFO* item, COLL_INFO* coll) {
	FLOOR_INFO* floor;
	height_types ht;
	int tiltxoff, tiltzoff, OnObject;
	short room_number;

	item->fallspeed = 0;
	item->gravity_status = 0;
	room_number = item->room_number;
	floor = GetFloor(item->pos.pos.x, item->pos.pos.y, item->pos.pos.z, &room_number);
	GetHeight(floor, item->pos.pos.x, item->pos.pos.y, item->pos.pos.z, &ht, &tiltxoff, &tiltzoff, &OnObject);
	coll->trigger = trigger_index;

	if(!(S_IsActionDown(inputImpl, IN_ACTION)) || item->hit_points <= 0) {
		lara.torso_x_rot = 0;
		lara.torso_y_rot = 0;
		lara.head_x_rot = 0;
		lara.head_y_rot = 0;
		item->anim_number = ANIM_FALLDOWN;
		item->frame_number = GetAnim(currentLevel, ANIM_FALLDOWN)->frame_base;
		item->current_anim_state = AS_FORWARDJUMP;
		item->goal_anim_state = AS_FORWARDJUMP;
		item->speed = 2;
		item->gravity_status = 1;
		item->fallspeed = 1;
		lara.gun_status = LG_NO_ARMS;
		return 1;
	}

	return 0;
}

int LaraClimbLeftCornerTest(ITEM_INFO* item, COLL_INFO* coll) {
	int flag, oldX, oldZ, x, z, shift;
	short oldY, angle;

	flag = 0;

	if(item->anim_number != 171) {
		return 0;
	}

	oldX = item->pos.pos.x;
	oldY = item->pos.y_rot;
	oldZ = item->pos.pos.z;
	angle = (unsigned short)(item->pos.y_rot + 0x2000) / 0x4000;

	switch(angle) {
	case NORTH:
	case SOUTH:
		x = (oldX & ~0x3FF) - (oldZ & 0x3FF) + 1024;
		z = (oldZ & ~0x3FF) - (oldX & 0x3FF) + 1024;
		break;

	default:
		x = (oldX & ~0x3FF) + (oldZ & 0x3FF);
		z = (oldZ & ~0x3FF) + (oldX & 0x3FF);
		break;
	}

	if(GetClimbTrigger(x, item->pos.pos.y, z, item->room_number) & LeftIntRightExtTab[angle]) {
		item->pos.pos.x = x;
		item->pos.pos.z = z;
		lara.CornerX = x;
		lara.CornerZ = z;
		item->pos.y_rot -= 0x4000;
		lara.move_angle = item->pos.y_rot;
		flag = LaraTestClimbPos(item, coll->radius, -120 - coll->radius, -512, 512, &shift);
		item->item_flags[3] = (short)flag;

		if(flag) {
			flag = -1;
		}
	}

	if(!flag) {
		item->pos.pos.x = oldX;
		item->pos.y_rot = oldY;
		item->pos.pos.z = oldZ;
		lara.move_angle = oldY;

		switch(angle) {
		case NORTH:
			x = (item->pos.pos.x ^ (((unsigned short)item->pos.pos.z ^ (unsigned short)item->pos.pos.x) & 0x3FF)) - 1024;
			z = (((unsigned short)item->pos.pos.z ^ (unsigned short)item->pos.pos.x) & 0x3FF) ^ (item->pos.pos.z + 1024);
			break;

		case SOUTH:
			x = (((unsigned short)item->pos.pos.z ^ (unsigned short)item->pos.pos.x) & 0x3FF) ^ (item->pos.pos.x + 1024);
			z = (((unsigned short)item->pos.pos.z ^ (unsigned short)item->pos.pos.x) & 0x3FF) ^ (item->pos.pos.z - 1024);
			break;

		case WEST:
			x = (item->pos.pos.x & ~0x3FFu) - (item->pos.pos.z & 0x3FF);
			z = (item->pos.pos.z & ~0x3FFu) - (item->pos.pos.x & 0x3FF);
			break;

		default:
			x = ((item->pos.pos.x + 1024) & ~0x3FF) - (item->pos.pos.z & 0x3FF) + 1024;
			z = ((item->pos.pos.z + 1024) & ~0x3FF) - (item->pos.pos.x & 0x3FF) + 1024;
			break;
		}

		if(GetClimbTrigger(x, item->pos.pos.y, z, item->room_number) & LeftExtRightIntTab[angle]) {
			item->pos.pos.x = x;
			item->pos.pos.z = z;
			lara.CornerX = x;
			lara.CornerZ = z;
			item->pos.y_rot += 0x4000;
			lara.move_angle = item->pos.y_rot;
			flag = LaraTestClimbPos(item, coll->radius, -120 - coll->radius, -512, 512, &shift);
			item->item_flags[3] = (short)flag;

			if(flag) {
				flag = 1;
			}
		}
	}

	item->pos.pos.x = oldX;
	item->pos.y_rot = oldY;
	item->pos.pos.z = oldZ;
	lara.move_angle = oldY;
	return flag;
}

int LaraClimbRightCornerTest(ITEM_INFO* item, COLL_INFO* coll) {
	int flag, oldX, oldZ, x, z, shift;
	short oldY, angle;

	flag = 0;

	if(item->anim_number != 170) {
		return 0;
	}

	oldX = item->pos.pos.x;
	oldY = item->pos.y_rot;
	oldZ = item->pos.pos.z;
	angle = (unsigned short)(item->pos.y_rot + 0x2000) / 0x4000;

	switch(angle) {
	case NORTH:
	case SOUTH:
		x = (oldX & ~0x3FF) + (oldZ & 0x3FF);
		z = (oldZ & ~0x3FF) + (oldX & 0x3FF);
		break;

	default:
		x = (oldX & ~0x3FF) - (oldZ & 0x3FF) + 1024;
		z = (oldZ & ~0x3FF) - (oldX & 0x3FF) + 1024;
		break;
	}

	if(GetClimbTrigger(x, item->pos.pos.y, z, item->room_number) & LeftExtRightIntTab[angle]) {
		item->pos.pos.x = x;
		item->pos.pos.z = z;
		lara.CornerX = x;
		lara.CornerZ = z;
		item->pos.y_rot += 0x4000;
		lara.move_angle = item->pos.y_rot;
		flag = LaraTestClimbPos(item, coll->radius, coll->radius + 120, -512, 512, &shift);

		if(flag) {
			flag = -1;
		}
	}

	if(!flag) {
		item->pos.pos.x = oldX;
		item->pos.y_rot = oldY;
		item->pos.pos.z = oldZ;
		lara.move_angle = oldY;

		switch(angle) {
		case NORTH:
			x = ((item->pos.pos.x + 1024) & ~0x3FF) - (item->pos.pos.z & 0x3FF) + 1024;
			z = ((item->pos.pos.z + 1024) & ~0x3FF) - (item->pos.pos.x & 0x3FF) + 1024;
			break;

		case SOUTH:
			x = ((item->pos.pos.x - 1024) & ~0x3FF) - (item->pos.pos.z & 0x3FF) + 1024;
			z = ((item->pos.pos.z - 1024) & ~0x3FF) - (item->pos.pos.x & 0x3FF) + 1024;
			break;

		case WEST:
			x = (item->pos.pos.x ^ ((unsigned short)item->pos.pos.z ^ (unsigned short)item->pos.pos.x) & 0x3FF) - 1024;
			z = ((unsigned short)item->pos.pos.z ^ (unsigned short)item->pos.pos.x) & 0x3FF ^ (item->pos.pos.z + 1024);
			break;

		default:
			x = (((unsigned short)item->pos.pos.z ^ (unsigned short)item->pos.pos.x) & 0x3FF) ^ (item->pos.pos.x + 1024);
			z = (item->pos.pos.z ^ (((unsigned short)item->pos.pos.z ^ (unsigned short)item->pos.pos.x) & 0x3FF)) - 1024;
			break;
		}

		if(GetClimbTrigger(x, item->pos.pos.y, z, item->room_number) & LeftIntRightExtTab[angle]) {
			item->pos.pos.x = x;
			item->pos.pos.z = z;
			lara.CornerX = x;
			lara.CornerZ = z;
			item->pos.y_rot -= 0x4000;
			lara.move_angle = item->pos.y_rot;
			flag = LaraTestClimbPos(item, coll->radius, coll->radius + 120, -512, 512, &shift);

			if(flag) {
				flag = 1;
			}
		}
	}

	item->pos.pos.x = oldX;
	item->pos.y_rot = oldY;
	item->pos.pos.z = oldZ;
	lara.move_angle = oldY;
	return flag;
}

void LaraDoClimbLeftRight(ITEM_INFO* item, COLL_INFO* coll, int result, int shift) {
	int flag;

	if(result == 1) {
		if(S_IsActionDown(inputImpl, IN_LEFT)) {
			item->goal_anim_state = AS_CLIMBLEFT;
		} else if(S_IsActionDown(inputImpl, IN_RIGHT)) {
			item->goal_anim_state = AS_CLIMBRIGHT;
		} else {
			item->goal_anim_state = AS_CLIMBSTNC;
		}

		item->pos.pos.y += shift;
		return;
	} else if(!result) {
		item->pos.pos.x = coll->old.x;
		item->pos.pos.z = coll->old.z;
		item->current_anim_state = AS_CLIMBSTNC;
		item->goal_anim_state = AS_CLIMBSTNC;

		if(coll->old_anim_state != AS_CLIMBSTNC) {
			item->anim_number = ANIM_CLIMBSTNC;
			item->frame_number = GetAnim(currentLevel, ANIM_CLIMBSTNC)->frame_base;
			return;
		}

		if(S_IsActionDown(inputImpl, IN_LEFT)) {
			flag = LaraClimbLeftCornerTest(item, coll);

			if(flag) {
				if(flag > 0) {
					item->anim_number = ANIM_EXTCLIMBL;
					item->frame_number = GetAnim(currentLevel, ANIM_EXTCLIMBL)->frame_base;
					item->current_anim_state = AS_CORNEREXTL;
					item->goal_anim_state = AS_CORNEREXTL;
				} else {
					item->anim_number = ANIM_INTCLIMBL;
					item->frame_number = GetAnim(currentLevel, ANIM_INTCLIMBL)->frame_base;
					item->current_anim_state = AS_CORNERINTL;
					item->goal_anim_state = AS_CORNERINTL;
				}

				return;
			}
		} else if(S_IsActionDown(inputImpl, IN_RIGHT)) {
			flag = LaraClimbRightCornerTest(item, coll);

			if(flag) {
				if(flag > 0) {
					item->anim_number = ANIM_EXTCLIMBR;
					item->frame_number = GetAnim(currentLevel, ANIM_EXTCLIMBR)->frame_base;
					item->current_anim_state = AS_CORNEREXTR;
					item->goal_anim_state = AS_CORNEREXTR;
				} else {
					item->anim_number = ANIM_INTCLIMBR;
					item->frame_number = GetAnim(currentLevel, ANIM_INTCLIMBR)->frame_base;
					item->current_anim_state = AS_CORNERINTR;
					item->goal_anim_state = AS_CORNERINTR;
				}

				return;
			}
		}

		item->frame_number = coll->old_frame_number;
		item->anim_number = coll->old_anim_number;
		AnimateLara(item);
	} else {
		item->goal_anim_state = AS_HANG;

		do {
			AnimateItem(item);
		} while(item->current_anim_state != AS_HANG);

		item->pos.pos.x = coll->old.x;
		item->pos.pos.z = coll->old.z;
	}
}
