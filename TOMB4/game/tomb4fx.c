...
void LaraBubbles(ITEM_INFO* item) {
	...
	SoundEffect(SFX_LARA_BUBBLES, (PHD_VECTOR*)&item->pos, item->room_number, SFX_WATER);
	pos.x = 0;
	pos.y = -4;
	pos.z = 64;
...
void UpdateGunShells() {
	...
	for(int i = 0; i < nGunshells; i++) {
		...
		if(shell->pos.pos.y < c) {
			SoundEffect(SFX_LARA_SHOTGUN_SHELL, (PHD_VECTOR*)&shell->pos, shell->room_number, SFX_DEFAULT);
			shell->speed -= 4;
