					...
					if(dx > -2048 && dx < 2048 && dz > -2048 && dz < 2048
					   && dy > -2048 && dy < 2048) {
						if(TestBoundsCollide(item, bike, 500)) {
							...
							if(item->hit_points) {
								SoundEffect(
									SFX_BIKE_HIT_ENEMIES, (PHD_VECTOR*)&item->pos, item->room_number,
									SFX_DEFAULT);
							}
				...
				if(mesh->static_number >= SHATTER0
				   && mesh->static_number <= SHATTER9) {
					...
					if(BikeBounds[0] > CollidedStaticBounds[1]
						...
						SoundEffect(
							SFX_HIT_ROCK, &pos, rn, SFX_DEFAULT);
...
void BikeControl(short item_number) {
	...
	if(bike->velocity > 0 || bike->unused1) {
		...
		SoundEffect(
			SFX_BIKE_MOVING, (PHD_VECTOR*)&item->pos, item->room_number,
			(bike->pitch2 << 8) + (SFX_SETPITCH | 0x1000000));
	} else {
		if(driving != -1) {
			SoundEffect(SFX_BIKE_IDLE, (PHD_VECTOR*)&item->pos, item->room_number, SFX_DEFAULT);
			SoundEffect(
				SFX_BIKE_MOVING, (PHD_VECTOR*)&item->pos, item->room_number,
				(bike->pitch2 << 8) + (SFX_SETPITCH | 0x1000000));
		}
