...
void SarcophagusCollision(short item_number, ITEM_INFO* l, COLL_INFO* coll) {
	...
	if((S_IsActionDown(inputImpl, IN_ACTION) && item->status != ITEM_ACTIVE && l->current_anim_state == AS_STOP && l->anim_number == ANIM_BREATH && lara.gun_status == LG_NO_ARMS) || (lara.IsMoving && lara.GeneralPtr == item_number)) {
		if(TestLaraPosition(SarcophagusBounds, item, l)) {
			if(MoveLaraPosition(&SarcophagusPos, item, l)) {
				...
				l->frame_number = GetAnim(currentLevel, ANIM_SARCOPHAGUS)->frame_base;
	...
	} else if(l->anim_number == ANIM_SARCOPHAGUS && l->frame_number == GetAnim(currentLevel, ANIM_SARCOPHAGUS)->frame_base + 113) {
...
void PuzzleDoneCollision(short item_num, ITEM_INFO* l, COLL_INFO* coll) {
	if(GetItem(currentLevel, item_num)->trigger_flags != 999) {
...
void AnimatingPickUp(short item_number) {
	if((GetItem(currentLevel, item_number)->trigger_flags & 0x3F) == 2) {
		AnimateItem(GetItem(currentLevel, item_number));
	}
}
...
void RegeneratePickups() {
	...
	for(int i = 0; i < NumRPickups; i++) {
		item = GetItem(currentLevel, RPickups[i]);
...
void PickUpCollision(short item_number, ITEM_INFO* l, COLL_INFO* coll) {
	...
	if(lara.water_status == LW_ABOVE_WATER || lara.water_status == LW_WADE) {
		if(((S_IsActionDown(inputImpl, IN_ACTION) || (GLOBAL_inventoryitemchosen != NO_ITEM && ocb == 2)) && !BinocularRange && ((l->current_anim_state == AS_STOP && l->anim_number == ANIM_BREATH && lara.gun_status == LG_NO_ARMS) || (l->current_anim_state == AS_DUCK && l->anim_number == ANIM_DUCKBREATHE && lara.gun_status == LG_NO_ARMS) || (l->current_anim_state == AS_ALL4S && l->anim_number == ANIM_ALL4S))) || lara.IsMoving && lara.GeneralPtr == item_number) {
			...
			if(l->frame_number == GetAnim(currentLevel, ANIM_PICKUP)->frame_base + 15 || l->frame_number == GetAnim(currentLevel, ANIM_DUCKPICKUP)->frame_base + 22 || l->frame_number == GetAnim(currentLevel, ANIM_DUCKPICKUP)->frame_base + 20 || l->frame_number == GetAnim(currentLevel, ANIM_PLINTHLO)->frame_base + 29 || l->frame_number == GetAnim(currentLevel, ANIM_PLINTHHI)->frame_base + 45 || l->frame_number == GetAnim(currentLevel, ANIM_HIDDENPICKUP)->frame_base + 42 || (l->anim_number == ANIM_CROWBAR && l->frame_number == GetAnim(currentLevel, ANIM_CROWBAR)->frame_base + 123)) {
