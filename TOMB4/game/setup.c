...
void BaddyObjects() {
	...
	*GetBone(currentLevel, obj->bone_index + 48) |= 4;
	...
	if(obj->loaded) {
		...
		*GetMeshPointer(currentLevel, obj->mesh_index + 15) = GetMesh(
			currentLevel,
			GetObjectInfo(currentLevel, MESHSWAP1)->mesh_index + 14);
		*GetMeshPointer(currentLevel, obj->mesh_index + 31) = GetMesh(
			currentLevel,
			GetObjectInfo(currentLevel, MESHSWAP1)->mesh_index + 30);
		*GetMeshPointer(currentLevel, obj->mesh_index + 37) = GetMesh(
			currentLevel,
			GetObjectInfo(currentLevel, MESHSWAP1)->mesh_index + 36);
	}
	...
	if(obj->loaded) {
		...
		*GetMeshPointer(currentLevel, obj->mesh_index + 31) = GetMesh(
			currentLevel,
			GetObjectInfo(currentLevel, MESHSWAP2)->mesh_index + 30);
		*GetMeshPointer(currentLevel, obj->mesh_index + 37) = GetMesh(
			currentLevel,
			GetObjectInfo(currentLevel, MESHSWAP2)->mesh_index + 36);
		*GetMeshPointer(currentLevel, obj->mesh_index + 43) = GetMesh(
			currentLevel,
			GetObjectInfo(currentLevel, MESHSWAP2)->mesh_index + 42);
	}
	...
	if(obj->loaded) {
		...
		*GetMeshPointer(currentLevel, obj->mesh_index + 9) = GetMesh(
			currentLevel,
			GetObjectInfo(currentLevel, MESHSWAP3)->mesh_index + 8);
		*GetMeshPointer(currentLevel, obj->mesh_index + 15) = GetMesh(
			currentLevel,
	...
	if(obj->loaded) {
		...
		*GetMeshPointer(currentLevel, obj->mesh_index + 9) = GetMesh(
			currentLevel,
			GetObjectInfo(currentLevel, MESHSWAP2)->mesh_index + 8);
		*GetMeshPointer(currentLevel, obj->mesh_index + 15) = GetMesh(
			currentLevel,
			...
			for(int i = 0; i < GetNumRooms(lvl); i++) {
				...
				for(int ry = 0; ry < 4; ry++) {
					for(int rx = 0; rx < 4; rx++) {
						if(x + rx >= roomx && x + rx < roomx + r->x_size - 2
						   && y + ry >= roomy
						   && y + ry < roomy + r->y_size - 2) {
							cont = 1;
							break;
						}
					}
				...
				if(cont) {
					pTable = (unsigned char*)&OutsideRoomTable
						[1728 * (y >> 2) + 64 * (x >> 2)];
...
void InitialiseObjects() {
	...
	for(int i = 0; i < gfNumMips; i++) {
		obj = GetObjectInfo(
			currentLevel, ((gfMips[i] & 0xF) << 1) + ANIMATING1);
...
void GetCarriedItems() {
	...
	for(int i = 0; i < GetNumLevelItems(currentLevel); i++) {
		baddy = GetItem(currentLevel, i);
		baddy->carried_item = NO_ITEM;

		if(GetObjectInfo(currentLevel, baddy->object_number)->intelligent
		   && baddy->object_number != SCORPION) {
...
void InitTarget() {
	...
	meshpp = GetMeshPointer(
		currentLevel, GetObjectInfo(currentLevel, TARGET_GRAPHICS)->mesh_index);
	meshp = *meshpp;
