...
void GameStixControl(short item_number) {
	...
	if(item->trigger_flags > -1) {
		if(item->hit_points == 100) {
			SoundEffect(SFX_SPINNING_PUZZLE, (PHD_VECTOR*)&item->pos, item->room_number, SFX_DEFAULT);
		}
...
void GameStixCollision(short item_number, ITEM_INFO* l, COLL_INFO* coll) {
	...
	if(S_IsActionDown(inputImpl, IN_ACTION) && l->current_anim_state == AS_STOP && l->anim_number == ANIM_BREATH && lara.gun_status == LG_NO_ARMS && !item->active || lara.IsMoving && lara.GeneralPtr == item_number) {
