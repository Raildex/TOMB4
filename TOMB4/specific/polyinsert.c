...
void HWR_DrawSortList(D3DTLBUMPVERTEX* info, short num_verts, short texture, short type) {
	...
	case 0:
		...
		DXAttempt(IDirect3DDevice3_SetTexture(App.dx.lpD3DDevice,0, Textures[texture].tex));
	...
	if(GlobalFogOff)
		...
		if(dist > 0x19000000)
			...
			if(FogBulb->inRange) {
				...
				FogBulb->dist = (float)sqrt(SQUARE(FogBulb->pos.x) + SQUARE(FogBulb->pos.y) + SQUARE(FogBulb->pos.z));
			}
		}
	}
}
